...
public class Chapter02 {
            ...
            throws InterruptedException
    {
        ...
        long s = conn.hlen("login:");//统计散列的数量
    ...
    public void updateToken(Jedis conn, String token, String user, String item) {
        ...
        conn.hset("login:", token, user); //保存登陆用户的集合
        conn.zadd("recent:", timestamp, token);  //保存用户登入时间的集合
        if (item != null) {
            conn.rpush("view:"+token,item);
            conn.ltrim("view:"+token,-25,-1);
//            conn.zadd("viewed:" + token, timestamp, item);  //保存用户浏览商品的集合
//            conn.zremrangeByRank("viewed:" + token, 0, -26);  //只保存最新的25个商品
//            Double zincrby = conn.zincrby("viewed:", -1, item);
//            System.out.println("zincrby:"+zincrby);
            ...
            extends Thread
    {
        ...
        public void run() {
            while (!quit) {
                long size = conn.zcard("recent:"); //统计数量
                if (size <= limit){    //超过限制的数量就删除用户的的登陆信息，用户的登入时间信息，用户的浏览信息
                    try {
                        sleep(1000);
